...
func SendPCM(v *discordgo.VoiceConnection, pcm <-chan []int16) {
	// if pcm == nil means we have paused the audio, so dont end here
	// if pcm == nil {
	// 	return
	// }
...
func PlayAudioFile(v *discordgo.VoiceConnection, filename string, stop <-chan bool, state *PlaybackState) {
	...
	for {
		if state.Paused {
			// Playback is paused, wait until it's resumed
			state.cond.L.Lock()
			state.cond.Wait()
			state.cond.L.Unlock()
		} else {
